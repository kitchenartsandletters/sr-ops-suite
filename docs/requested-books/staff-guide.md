...
If it isn’t written in the thread, the system does not know it happened.
...
You do not need to mark every stage — only the important ones.
...
(Optionally: add a ✅ checkmark to the original message)
...
If fulfillment is not marked, the system treats the request as still open.
...
Please avoid the following:
	...
	•	 Do not leave a request silent after fulfillment
