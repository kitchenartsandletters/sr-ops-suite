...
const { SHOP_URL, SHOPIFY_ACCESS_TOKEN, SHOPIFY_API_VERSION, ISBN_PRICE_LOOKUP_SHARED_SECRET, PORT, } = process.env;
...
async function queryShopify(isbns) {
    const query = `
  ...
  `;
    ...
    return json.data.productVariants.edges.map((edge) => edge.node);
}
app.post('/isbn/prices', async (req, res) => {
    try {
